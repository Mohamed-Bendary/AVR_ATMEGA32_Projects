...
void LCD_voidInit(void)//hena ha7ot al direction bta3 pins al data wel control
{
	...
	/*5aly balak wenta betwasal*/
	DIO_voidSetPinDirection(LCD_Data_Port,DIO_u8PIN0,DIO_u8PORT_OUTPUT);//Pin 0 ma3 DB4
	DIO_voidSetPinDirection(LCD_Data_Port,DIO_u8PIN1,DIO_u8PORT_OUTPUT);//Pin 1 ma3 DB5
	DIO_voidSetPinDirection(LCD_Data_Port,DIO_u8PIN2,DIO_u8PORT_OUTPUT);//Pin 2 ma3 DB6
	DIO_voidSetPinDirection(LCD_Data_Port,DIO_u8PIN3,DIO_u8PORT_OUTPUT);//Pin 3 ma3 DB7
	/*to initalize LCD (from data sheet)first is to wait(delay)more than 30ms after the power is on*/
	_delay_ms(40);
	/*SEnd this command twice,0b0010xxxx*/
	LCD_voidSendCommand(0b00100000);
	LCD_voidSendCommand(0b00100000);
	/*Number of lines and font size,0bNFxxxxxx*/
	LCD_voidSendCommand(0b11000000);


	/*Display ON/OFF */
	LCD_voidSendCommand(0b00000000);
	/*Display configurations, 0b1DCBxxxx*/
	LCD_voidSendCommand(0b10000000);//disaple cursor and its blinking
...
void LCD_voidPosition(u8 Copy_u8RowPos,u8 Copy_u8ColPos){
	u8 Local_u8Address;
	/*Setting the address location if the user wants to set the postion in the first row*/
	if(Copy_u8RowPos==0){
